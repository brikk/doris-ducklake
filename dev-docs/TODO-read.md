...
- [x] **Table-level `getTableStatistics`** — done. `DuckLakeConnectorMetadata.getTableStatistics` returns `ConnectorTableStatistics(recordCount, fileSizeBytes)` from `ducklake_table_stats` (`DucklakeCatalog.getTableStats`). Whole-table for now; the planner applies pushed-filter selectivity on top. fe-core also falls back to `dataSize / rowWidth` if only a size is present, but we supply an exact `recordCount`.
 ...
 - [x] **global-stats-time-travel (correctness, from 2026-08-26 upstream survey) — FIXED 2026-08-26.** `getTableStatistics` (`DuckLakeConnectorMetadata.kt`) returned `catalog.getTableStats(dlHandle.tableId)` — the **whole-table/global** `ducklake_table_stats` row — ignoring `dlHandle.snapshotId`. Since Step 8 time travel IS wired, a `FOR VERSION/TIME AS OF` query reported the **current** snapshot's row count/size for a **historical** read → misled planner cardinality. Same class as upstream DuckLake `d2f4b9c0` ("Only apply global statistics to current-snapshot table scans"). **Fix shipped (snapshot-scoped, option (b)):** current-snapshot pin → serve the cheap global
`ducklake_table_stats` row; **historical pin → derive the estimate from the data files live at the pin**
(`catalog.getDataFiles(tableId, snapshotId)`, sum `recordCount` / `fileSizeBytes`), so time-travel reads
keep real cardinality instead of the current-snapshot value or planner defaults. Estimate caveat: does
not subtract position-deletes (neither does the whole-table row) and excludes inlined data. Test:
`DuckLakeConnectorMetadataTimeTravelTest.tableStatisticsAreSnapshotScopedForTimeTravelPin` (advances
history +3 rows; asserts historical pin reports the OLD count, `< rows1`). This was the **consumer** half
of a shared finding — the catalog is fine; the same item exists on the Trino side.
