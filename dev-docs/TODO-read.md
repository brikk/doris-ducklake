...
- [x] **Table-level `getTableStatistics`** — done. `DuckLakeConnectorMetadata.getTableStatistics` returns `ConnectorTableStatistics(recordCount, fileSizeBytes)` from `ducklake_table_stats` (`DucklakeCatalog.getTableStats`). Whole-table for now; the planner applies pushed-filter selectivity on top. fe-core also falls back to `dataSize / rowWidth` if only a size is present, but we supply an exact `recordCount`.
 ...
 - [ ] **global-stats-time-travel (correctness, from 2026-08-26 upstream survey).** `getTableStatistics` (`DuckLakeConnectorMetadata.kt:339`) returns `catalog.getTableStats(dlHandle.tableId)` — the **whole-table/global** `ducklake_table_stats` row — but ignores `dlHandle.snapshotId`. Since Step 8 time travel IS wired, a `FOR VERSION/TIME AS OF` query now reports the **current** snapshot's row count/size for a **historical** read, which can badly mislead planner cardinality. Upstream DuckLake just fixed the same class of bug (`ducklake/main` `d2f4b9c0` "Only apply global statistics to current-snapshot table scans"). Fix: when the handle is pinned to a non-current snapshot, either (a) skip global stats (`return Optional.empty()`), or (b) derive a snapshot-scoped row count from `catalog.getColumnStats(tableId, snapshotId)` (already aggregates value counts under `activeAt(snapshotId)`) / sum `DucklakeDataFile.recordCount` for the pinned file set. Cheapest correct v1: only serve `getTableStats` when the pin equals the current snapshot; otherwise empty. NB this is the **consumer** half of a shared finding — the *catalog* is fine (it offers both global and snapshot-scoped APIs); the same item exists on the Trino side. ~1h.
