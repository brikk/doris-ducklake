...
- [x] **hive-layout `add_files` path partition fill** (was `GAP_HIVE_PARTITION_FILL`;
  shipped 2026-07-11). Files registered from a hive directory layout
  (`part_key=1/part_key2=10/f.parquet`) carry partition columns in the PATH, not
  the parquet body (`is_partition` name-map entries via `getPartitionNameMaps`).
  `DuckLakeScanPlanProvider` now: (1) detects hive-layout files per-file by
  `mappingId` + partition name map; (2) parses the values from the path
  (`HivePartitionPath`, mirroring trino) and emits them as `columns_from_path`
  on the range PLUS a scan-node `path_partition_keys` declaration
  (`PluginDrivenScanNode.getPathPartitionKeys` → `classifyColumn` excludes them
  from the file/decode set) — WITHOUT the latter the BE reads the body-absent
  column and fails "name_mapping must be set"; (3) excludes those columns from
  the field-id schema dictionary. Verified live: `add_files_hive.test` +
  `add_files_hive_many_columns.test` GREEN (`corpusReplayTest -Dducklake.corpus.dirs=add_files`,
  456 passed).
  - **Known BE-side limits** (cast test stays skipped, root-caused): the BE's
    `columns_from_path` on the iceberg-dispatched reader (a) cannot represent a
    NULL hive partition value (the `__HIVE_DEFAULT_PARTITION__` sentinel / is-null
    flag are NOT converted to SQL NULL, and a NULL predicate over it crashes the
    reader) — the connector FAILS LOUD on these (`DorisConnectorException`,
    engine-skip); and (b) does not type a BOOLEAN path-partition column for
    boolean ops. Non-null int/string/date/decimal/timestamp/float/special-char
    path partitions all read correctly. Upstream: needs the BE fill to honor
    `columns_from_path_is_null` + hive sentinel, and boolean cast-from-path.
  - **Mixed-layout guard:** a table mixing hive-layout add_files files with
    natively-written partitioned files (same columns in the body) throws
    (`path_partition_keys` is scan-node-wide) rather than silently wrong-fill.
...
- [x] **add_files-without-stats — correctness handled.** `findDataFileIdsInRange` uses a **LEFT JOIN** (not inner) and RETAINS any file with no `file_column_stats` row for the predicate column (`no stats row -> unknown -> keep`, `JdbcDucklakeCatalog.kt:950-984`, referencing issue 1135). So a no-stats `add_files` file is never wrongly pruned out — only left unpruned. *(Remaining, deferred optimization, low value:* prune a no-stats IDENTITY-partitioned file by its recorded `getFilePartitionValues` value instead of keeping it — the bucket path already does this for BUCKET specs. Edge case: hive `add_files` files DO get derived partition stats, so this only bites niche no-stats identity files. Revisit only if hot — adds a per-identity-field prune mirroring `bucketPrune`.)*
...
- [ ] **float-nan-prune-guard — inherited from the shared catalog; do NOT reimplement here (2026-08-26 survey).** Our file pruning goes entirely through `catalog.findDataFileIdsInRange` (`DuckLakeConnectorMetadata.kt:245`), so the float NaN pruning bug (catalog float min/max exclude NaN, NaN sorts above all values → `col > C` can wrongly prune a `REAL`/`DOUBLE` file; cf. `datafusion-ducklake` #203) is being fixed **inside `ducklake-catalog`** (`dev-docs/TODO-UPSTREAM-2026-08.md` P1). Doris gets it for free on the next catalog bump — no Doris-side change. Same for the DECIMAL swapped-min>max guard (already in the catalog). Just bump the `ducklake-catalog` dependency when the fix lands.
...
- [x] **Table-level `getTableStatistics`** — done. `DuckLakeConnectorMetadata.getTableStatistics` returns `ConnectorTableStatistics(recordCount, fileSizeBytes)` from `ducklake_table_stats` (`DucklakeCatalog.getTableStats`). Whole-table for now; the planner applies pushed-filter selectivity on top. fe-core also falls back to `dataSize / rowWidth` if only a size is present, but we supply an exact `recordCount`.
 ...
 - [x] **global-stats-time-travel (correctness, from 2026-08-26 upstream survey) — FIXED 2026-08-26.** `getTableStatistics` (`DuckLakeConnectorMetadata.kt`) returned `catalog.getTableStats(dlHandle.tableId)` — the **whole-table/global** `ducklake_table_stats` row — ignoring `dlHandle.snapshotId`. Since Step 8 time travel IS wired, a `FOR VERSION/TIME AS OF` query reported the **current** snapshot's row count/size for a **historical** read → misled planner cardinality. Same class as upstream DuckLake `d2f4b9c0` ("Only apply global statistics to current-snapshot table scans"). **Fix shipped (snapshot-scoped, option (b)) + live-validated.** current-snapshot pin → serve the cheap
global `ducklake_table_stats` row; **historical pin → derive from the data files live at the pin**
(`catalog.getDataFiles(tableId, snapshotId)`, sum `recordCount` / `fileSizeBytes`). **Key:** fe-core
threads the pinned snapshot through the **3-arg `getTableStatistics(session, handle, snapshot)`** overload
(`PluginDrivenExternalTable.fetchRowCountAtSnapshot`) — the `handle` there is resolved at the CURRENT
snapshot, so the 2-arg override alone was ignored for time travel (default 3-arg drops the snapshot). We
now override **both**; the 3-arg uses `snapshot.snapshotId`. Estimate caveat: doesn't subtract
position-deletes (neither does the whole-table row) and excludes inlined data. Tests:
`DuckLakeConnectorMetadataTimeTravelTest.tableStatisticsAreSnapshotScopedForTimeTravelPin` (covers 2-arg
historical handle AND 3-arg current-handle+pinned-snapshot). **Live-validated on master `1731787677f`**:
`EXPLAIN SELECT *` shows `cardinality=5` current vs **`cardinality=2` `FOR VERSION AS OF <snap>`** (splits
were already snapshot-scoped; cardinality now is too). Consumer half of a shared finding — the catalog is
fine; the same item exists on the Trino side.
...
- [x] **`SPI_READY_TYPES` whitelist removal — DONE upstream (#66135, 2026-07-27, verified live patch-free).** `CatalogFactory.SPI_READY_TYPES` is gone: a registered `ConnectorProvider` claiming its `getType()` is now sufficient. The companion `CreateTableInfo.pluginCatalogTypeToEngine` anchor also went away (`ENGINE=` optional/connector-owned via `ConnectorProvider.acceptedCreateTableEngineNames()`, default empty). Both FE-patch anchors removed → first PATCH-FREE build (pin `a0c10f0672b`). Verified live: `CREATE CATALOG type=ducklake` + bucket-partitioned no-`ENGINE=` `CREATE TABLE` both succeed unpatched; `SHOW TABLE STATUS` shows `Engine: ducklake`. `ducklake-fe.patch` now historical.
